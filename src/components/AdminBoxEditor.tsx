      ...
      <div className="flex gap-3">
        {!props.disabled && (
          <button
            ...
            onClick={async () => { setError(null); setSaving(true); try { const res = await fetch(`/api/admin/boxes/${props.boxId}`, { method: "PUT", headers: { "content-type": "application/json" }, body: JSON.stringify({ contentText }) }); const data = await res.json().catch(() => ({})); if (!res.ok) return setError(data?.error ?? "保存失败"); router.refresh(); } finally { setSaving(false); } }}
